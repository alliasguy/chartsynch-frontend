...
const UserdashboardKyc = () => {
  ...
  const uploadDocument = async (file, fieldName) => {
    ...
    try {
      ...
      if (data.secure_url) {
        ...
        Swal.fire('Upload Failed', data.error?.message || 'Document upload failed, please try a different file', 'error');
      }
    } catch (error) {
  ...
  const validateStep = (step) => {
    switch (step) {
      case 1:
        return formData.dateOfBirth && formData.nationality && formData.city;
      case 2:
  ...
  const handleSubmit = async (e) => {
    ...
    setLoader(true);
    try {
      const { status, data } = await apiFetchWithStatus('/api/submitKYC', {
        method: 'POST',
        body: formData,
      });
      ...
      if (data.status === 'ok') {
        ...
        logout();
        Swal.fire('Session Expired', 'Please log in again to submit your KYC.', 'warning').then(() => {
          navigate('/login');
        });
      } else {
