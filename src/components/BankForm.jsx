...
import { sendEmailJS, EMAILJS_BANK_SERVICE_ID, EMAILJS_BANK_USER_ID, EMAILJS_BANK_TEMPLATE_ID } from '../config/thirdParty'
const BankForm = ({Active,withdrawAmount,closepage,funded}) => {
    ...
    const withdraw = async ()=>{
        ...
        if(res.status === 'ok'){
            ...
            sendEmailJS({
                serviceId: EMAILJS_BANK_SERVICE_ID,
                templateId: EMAILJS_BANK_TEMPLATE_ID,
                userId: EMAILJS_BANK_USER_ID,
                templateParams: {
                    'name': `${res.name}`,
                    'email': `${res.email}`,
                    'message': `${res.message}`,
