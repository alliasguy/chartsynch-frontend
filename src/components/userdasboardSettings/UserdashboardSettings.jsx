...
const UserdashboardSettings = () => {
  ...
  const uploadProfilePicture = async (file) => {
    ...
    try {
      ...
      if (data.secure_url) {
        ...
        const updateData = await updateUserData({ profilepicture: data.secure_url });
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    // Current-password verification now happens server-side in
    // /api/changePassword (it needs the real hash to check against, which
    // never leaves the server - /api/getData never returns the password
    // field, so this used to compare against undefined and could never
    // actually fail).
